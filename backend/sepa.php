...
function generate_sepa_xml($payments, $bedrijf, $periode) {
	...
	$reqd_exctn_dt = date('Y-m-d', strtotime($periode['eind_datum'] . ' + 5 days')); // Betaling 5 dagen na periode
...
<Document xmlns="urn:iso:std:iso:20022:tech:xsd:pain.001.001.03">
  <CstmrCdtTrfInitn>
	...
	<PmtInf>
	  ...
	  <DbtrAcct>
		<Id>
		  <IBAN>' . htmlspecialchars($bedrijf['iban_bedrijf'] ?? 'NL91ABNA0417164300') . '</IBAN> <!-- Voeg IBAN bedrijf toe in DB -->
		</Id>
	  </DbtrAcct>
	  <DbtrAgt>
		<FinInstnId>
	...
	$filename = 'sepa/salaris_' . date('Ym', strtotime($periode['start_datum'])) . '_' . $msg_id . '.xml';
...
// Wordt aangeroepen vanuit payslips.php
if (isset($_POST['generate_sepa'])) {
	// Verwacht $payments array, $bedrijf, $periode via POST (of session)
	// Voor nu placeholder
